...
describe AWS::CF::Signer do
  shared_examples_for "CF signer" do
  ...
  context "using key_path" do
    ...
    it_behaves_like "CF signer"
  end

  context "using key_content" do
    let(:key_path) { File.expand_path(File.dirname(__FILE__) + '/keys/pk-APKAIKUROOUNR2BAFUUU.pem') }
    let(:key_content) { File.readlines(key_path).join("") }
